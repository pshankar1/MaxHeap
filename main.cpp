// Heap class
template <typename E, typename Comp> class Main {
private:
...
public:
Main(E* h, int num, int max) // Constructor
...
/*********
enqueue is initiated and it checkes if queue is full
********/
E enqueue(E ObjectID, int priority) {
	if(maxsize>=maxsize) {
		cout<<"queue full";
		return;
	}
	Heap[maxsize]=ObjectID;
	maxsize=maxsize+1;
  E  dequeue() {
	E maxQ=Heap[0];
	Heap[0]=Heap[size-1];
